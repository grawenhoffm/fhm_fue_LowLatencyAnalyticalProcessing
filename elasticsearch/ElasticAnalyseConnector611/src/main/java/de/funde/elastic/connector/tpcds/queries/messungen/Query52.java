 ...
 *
 */
public class Query52 implements IQuery {

	private final RestHighLevelClient client;
	private static final Logger LOG = LoggerFactory.getLogger(Query52.class);


	public Query52(RestHighLevelClient client) {
		this.client = client;
	}
	...
	public Optional<StopWatch> run(String indexname, int counter) {
		...
		try {
			...
			final StopWatch sw = new StopWatch("Query 52", ElasticAnalyseConnector.elasticVersion, ElasticAnalyseConnector.luceneVersion, ElasticAnalyseConnector.numberOfReplicas, ElasticAnalyseConnector.numberOfIndexShards, indexname, counter);
					        		...
					        		.field("ss_item_i_brand_id")
										...
										.subAggregation(AggregationBuilders.sum("ext_price")
											...
											.subAggregation(AggregationBuilders.topHits("additionalValues")
													.fetchSource(new String[] {"ss_sold_date_d_year", "ss_item_i_brand", "ss_item_i_brand_id"}, new String[0])
													.sorts(sortBuilders))));
				...
				for (final Bucket b : byProductNameAggregation.getBuckets()) {
					...
					for (final Bucket b1 : byBrandAggregation.getBuckets()) {
						...
						for (final Bucket b2 : byClassAggregation.getBuckets()) {
							...
							final Sum sum = b2.getAggregations().get("ext_price");
							...
							LOG.debug("Name: {}  \n  -  SUM: {} \n  -  {} ", b2.getKeyAsString(), sum.getValue(), builder.toString());
						}
					}
				}
			}
