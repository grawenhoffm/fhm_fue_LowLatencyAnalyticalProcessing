 ...
 *
 */
public class Query42 implements IQuery {

	private final RestHighLevelClient client;
	private static final Logger LOG = LoggerFactory.getLogger(Query42.class);


	public Query42(RestHighLevelClient client) {
		this.client = client;
	}
	...
	public Optional<StopWatch> run(String indexname, int counter) {
		...
		try {
			...
			final StopWatch sw = new StopWatch("Query 42", ElasticAnalyseConnector.elasticVersion, ElasticAnalyseConnector.luceneVersion, ElasticAnalyseConnector.numberOfReplicas, ElasticAnalyseConnector.numberOfIndexShards, indexname, counter);
...
//	                .must(new ScriptQueryBuilder(script1))
	                ...
	                .must(QueryBuilders.termQuery("ss_item_i_manager_id", 1))
			...
			sortBuilders.add(new FieldSortBuilder("ss_item_i_category_id").order(SortOrder.ASC));
			...
			aggregation.subAggregation(AggregationBuilders.terms("i_category_id")
					        .field("ss_item_i_category_id")
					        		...
					        		.field("ss_item_i_category.keyword")
										...
										.subAggregation(AggregationBuilders.sum("ext_price")
											...
											.subAggregation(AggregationBuilders.topHits("additionalValues")
													.fetchSource(new String[] {"ss_sold_date_d_year", "ss_item_i_category_id", "ss_item_i_category"}, new String[0])
													.sorts(sortBuilders))));
			...
			if (ElasticAnalyseConnector.WITH_RESULT_LOGGING) {
				...
				for (final Bucket b : byProductNameAggregation.getBuckets()) {
					final Terms byBrandAggregation = b.getAggregations().get("i_category_id");
					for (final Bucket b1 : byBrandAggregation.getBuckets()) {
						final Terms byClassAggregation = b1.getAggregations().get("i_category");
						for (final Bucket b2 : byClassAggregation.getBuckets()) {

							final StringBuilder builder = new StringBuilder();
							final Sum average = b2.getAggregations().get("ext_price");
