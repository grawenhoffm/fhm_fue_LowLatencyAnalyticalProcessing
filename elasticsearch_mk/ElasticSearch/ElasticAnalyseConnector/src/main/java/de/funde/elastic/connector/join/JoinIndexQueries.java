...
public class JoinIndexQueries implements IndexQueries {

	private final RestHighLevelClient client;
	private static final Logger LOG = LoggerFactory.getLogger(JoinIndexQueries.class);


	public JoinIndexQueries(RestHighLevelClient client) {
		this.client = client;
	}



	// Beispiel-Abfrage um an die verschiedenen Möglichkeiten der Resultsets und Attribute zu gelangen
	@Override
	public void getRequest() throws IOException {

		try {
		    final GetRequest request = new GetRequest(JoinIndexCreator.INDEX_NAME, "doc", "1");
...
//				sourceBuilder.timeout(new TimeValue(60, TimeUnit.SECONDS));
			...
			sourceBuilder.sort(new FieldSortBuilder("someValue").order(SortOrder.DESC));
			final String[] includeFields = new String[] {"someValue"};
			final String[] excludeFields = new String[0];// {"_type", "text"};
			sourceBuilder.fetchSource(includeFields, excludeFields);
			final SearchRequest searchRequest = new SearchRequest(JoinIndexCreator.INDEX_NAME);
			...
			for (final SearchHit hit : searchResponse.getHits().getHits()) {
				...
				sb1.append(hit.getSource().get("someValue")).append(" , ");
	...
	public void searchRequestWithParentFilterText() {
		...
		try {
			final SearchSourceBuilder sourceBuilder = new SearchSourceBuilder();
			sourceBuilder.query( new MatchQueryBuilder("my_join_field", "question"));
	...
	public void searchRequestWithParentIDFilter() {
		...
		try {
			final SearchSourceBuilder sourceBuilder = new SearchSourceBuilder();
			//sourceBuilder.query( new MatchQueryBuilder("my_join_field", "question"));
			sourceBuilder.query(new ParentIdQueryBuilder("answer", "4"));
	...
	/**
	 * Serachrequest mit Filter auf allen Antworten und Summe und AVG über alle Values
	 */
	@Override
	public void searchRequestWithParentIDFilterAndAggregation(String parentQuestionId) {
		...
		try {
			final SearchSourceBuilder sourceBuilder = new SearchSourceBuilder();
			sourceBuilder.query(new ParentIdQueryBuilder("answer", parentQuestionId));
	...
	/**
	 * Serachrequest mit Filter auf allen Antworten und Summe und AVG über alle Values nach dem Text groupiert.
	 */
	@Override
